...
import warnings
from dividefold.cogent.maths.stats.distribution import (
    chi_high,
    z_low,
    z_high,
    zprob,
...
from dividefold.cogent.maths.stats.special import (
    lgam,
    log_one_minus,
    one_minus_exp,
    MACHEP,
)
from dividefold.cogent.maths.stats.ks import psmirnov2x, pkstwo
from dividefold.cogent.maths.stats.kendall import pkendall, kendalls_tau
from dividefold.cogent.maths.stats.special import Gamma
...
from dividefold.cogent.maths.stats.util import Numbers
...
def G_ind(m, williams=False):
    ...
    if williams:
        q = 1 + (
            (tot * sum(1.0 / sum(m, 1)) - 1)
            * (tot * sum(1.0 / sum(m, 0)) - 1)
            / (6 * tot * df)
        )
        G = G / q
...
def chi_square_from_Dict2D(data):
    ...
    """
    test = sum(
        [((item[0] - item[1]) * (item[0] - item[1])) / item[1] for item in data.Items]
    )
    num_rows = len(data)
    ...
    try:
        ...
        if n1 < 2:
            return t_one_observation(
                sum(a), b, tails, exp_diff, none_on_zero_variance=none_on_zero_variance
            )
