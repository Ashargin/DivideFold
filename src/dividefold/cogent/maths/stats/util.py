...
from dividefold.cogent.util.misc import (
    FunctionWrapper,
    MappedList,
    MappedDict,
    ConstraintError,
)
from dividefold.cogent.util.table import Table
...
class SummaryStatistics(object):
    ...
    def __str__(self):
        ...
        if not result:
            return ""
        return str(
            Table("Statistic Value".split(), result, column_templates={"Value": "%.4g"})
        )
    ...
    def _get_variance(self):
        ...
        if self._variance is None:
            try:
                self._variance = self._standard_deviation * self._standard_deviation
            except TypeError:
...
class NumbersI(object):
    ...
    def summarize(self):
        ...
        return SummaryStatistics(
            Count=self.Count, Sum=self.Sum, Variance=self.Variance, Median=self.Median
        )
...
class Freqs(FreqsI, MappedDict):
    ...
    def __init__(self, data=None, Constraint=None, Mask=None, ValueMask=None):
        ...
        """
        super(Freqs, self).__init__(
            Constraint=Constraint, Mask=Mask, ValueMask=ValueMask
        )
        self += data
...
class NumberFreqs(NumberFreqsI, MappedDict):
    ...
    def __init__(self, data=None, Constraint=None, Mask=None, ValueMask=None):
        ...
        ValueMask:      function applied to values before addition
        """
        super(NumberFreqs, self).__init__(
            Constraint=Constraint, Mask=Mask, ValueMask=ValueMask
        )
        self += data
        r = self.RequiredKeys
