...
from dividefold.cogent.struct.rna2d import Pairs
from dividefold.cogent.util.dict2d import Dict2D
...
class PairedRegion(object):
    ...
    def __init__(self, Start, End, Length, Id=None):
        ...
        self.End = End
        if Length < 1:
            raise ValueError("PairedRegion should contain at least one base pair")
        self.Length = Length
        self.Id = Id
        self.Pairs = Pairs()
    ...
    def conflicting(self, other):
        ...
        if (
            ...
            other.Start < self.Start < other.End and other.Start < self.End < other.End
        ):
            return False
        return True
    ...
    def totalScore(self):
        ...
        for pr in self:
            try:
                ...
                raise ValueError("Score should be numerical, but is %s" % (pr.Score))
        return score
    ...
    def invertedEnumeratedBoundaries(self):
        ...
        for boundary_idx, boundary_value in eb.items():
            if boundary_value in result:
                raise ValueError("Boundary value %s is not unique" % (boundary_value))
        ...
        for begin_idx in range(end_idx - 1, -1, -1):
            ...
            if (begin_pos, end_pos) in pos_to_pr:
                ...
                for sol in bottom_left:
                    candidates.append(PairedRegions(sol + PairedRegions([this_region])))
...
def matrix_solutions(paired_regions, goal="max", scoring_function=num_bps):
    ...
    This methods fills a dynamic programming matrix (by calling
        ...
        top-right cell.
    """
    return dp_matrix_multi(
        paired_regions, goal=goal, scoring_function=scoring_function
    )[0][-1]
...
def opt_all(pairs, return_removed=False, goal="max", scoring_function=num_bps):
    ...
    for cl in cliques:
        new_result = []
        best = matrix_solutions(cl, goal=goal, scoring_function=scoring_function)
...
def opt_single_random(
    pairs, return_removed=False, goal="max", scoring_function=num_bps
):
    ...
    The user can specify the goal (maximize or minimize) and the scoring
        ...
        with the maximum number of base pairs.
    """
    nested_structs = opt_all(pairs, return_removed, goal, scoring_function)
    return choice(nested_structs)


def opt_single_property(
    pairs, return_removed=False, goal="max", scoring_function=num_bps
):
...
def conflict_elimination(pairs, sel_function, add_back=True, return_removed=False):
...
def inc_length(pairs, reversed=False, return_removed=False):
    ...
    lengths = sorted(length_pos_data)
...
def nussinov_traceback(m, i, j, pairs):
    ...
    for k in range(i, j):
        if m[i, j] == m[i, k] + m[k + 1, j]:
            return nussinov_traceback(m, i, k, pairs) | nussinov_traceback(
                m, k + 1, j, pairs
            )
