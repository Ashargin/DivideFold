...
from dividefold.cogent.util.transform import make_trans, float_from_string
from dividefold.cogent.core.tree import TreeNode, TreeError
from dividefold.cogent.util.misc import not_none, flatten
...
class Partners(list):
    ...
    __delitem__ = (
        ...
        __setslice__
    ) = append = extend = insert = pop = remove = reverse = sort = _not_implemented
    ...
    def toPartners(self, length, offset=0, strict=True):
        ...
        for up, down in self:
            ...
            if result[upstream] or result[downstream]:
                if strict:
                    raise ValueError("Pairs contain conflicting partners: %s" % self)
...
class StructureNode(TreeNode):
    ...
    def _get_type(self):
        ...
        raise ValueError(
            "_get_type failed on node with start %s, end %s" % (self.Start, self.End)
        )
    ...
    def pairChildren(self, first, second):
        ...
        new_node = StructureNode(Data=Stem(first_node.Start, second_node.Start, 1))
    ...
    def extendHelix(self, seq):
        ...
        while 1:
            ...
            if (
                indices
                and ((seq[indices[0]], seq[indices[1]]) in pairs)
...
def classify(struct, verbose=False):
    ...
    try:
        ...
        raise IndexError(
            "Trying to classify an invalid Vienna structure: %s" % (struct)
        )
