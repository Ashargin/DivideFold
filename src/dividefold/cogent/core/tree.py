...
import re
from dividefold.cogent.util.transform import comb
from dividefold.cogent.maths.stats.test import correlation
from operator import or_
from dividefold.cogent.util.misc import InverseDict
...
class TreeNode(object):
    ...
    def getNewickRecursive(
        self, with_distances=False, semicolon=True, escape_name=True
    ):
        ...
        subtrees = [
            child.getNewick(with_distances, semicolon=False) for child in self.Children
        ]
        if subtrees:
    ...
    def getEdgeNames(self, tip1name, tip2name, getclade, getstem, outgroup_name=None):
    ...
    def _getSubTree(self, included_names, constructor=None, keep_root=False):
        ...
        if self.Name in included_names:
            ...
            children = [
                child._getSubTree(included_names, constructor)
                for child in self.Children
            ]
    ...
    def asciiArt(self, show_internal=True, compact=False):
        ...
        """
        (lines, mid) = self._asciiArt(show_internal=show_internal, compact=compact)
        return "\n".join(lines)
    ...
    def getNodeMatchingName(self, name):
        ...
        if node is None:
            raise TreeError("No node named '%s' in %s" % (name, self.getTipNames()))
        return node
...
class PhyloNode(TreeNode):
    ...
    def compareByTipDistances(
        ...
        common_names = frozenset(self_names.keys()) & frozenset(other_names.keys())
