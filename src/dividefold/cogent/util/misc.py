...
from dividefold.cogent.util.option_parsing import parse_command_line_parameters
...
is_char_or_noniterable = lambda x: is_char(x) or not is_iterable(x)

is_str_or_noniterable = lambda x: isinstance(x, basestring) or not is_iterable(x)
...
class ConstrainedString(str, ConstrainedContainer):
    ...
    def __add__(self, other):
        ...
        result = self.__class__(
            str(self) + "".join(map(self.Mask, other)), Constraint=self.Constraint
        )
    ...
    def __mul__(self, multiplier):
        ...
        result = self.__class__(
            str.__mul__(self, multiplier), Constraint=self.Constraint
        )
        mask = self._mask_for_new()
        if mask:
            result.Mask = mask
    ...
    def __rmul__(self, multiplier):
        """Returns copy of self multiplied by multiplier."""
        result = self.__class__(
            str.__rmul__(self, multiplier), Constraint=self.Constraint
        )
        mask = self._mask_for_new()
        if mask:
            result.Mask = mask
    ...
    def __getslice__(self, *args, **kwargs):
        ...
        result = self.__class__(
            str.__getslice__(self, *args, **kwargs), Constraint=self.Constraint
        )
        mask = self._mask_for_new()
        if mask:
            result.Mask = mask
...
class ConstrainedList(ConstrainedContainer, list):
    ...
    def __add__(self, other):
        """Returns copy of self added to copy of other if constraint correct."""
        result = self.__class__(
            list(self) + map(self.Mask, other), Constraint=self.Constraint
        )
        mask = self._mask_for_new()
        if mask:
            result.Mask = mask
    ...
    def __getslice__(self, *args, **kwargs):
        """Make sure slice remembers the constraint."""
        result = self.__class__(
            list.__getslice__(self, *args, **kwargs), Constraint=self.Constraint
        )
        mask = self._mask_for_new()
        if mask:
            result.Mask = mask
...
class ConstrainedDict(ConstrainedContainer, dict):
    ...
    def copy(self):
        ...
        return self.__class__(
            self, Constraint=self.Constraint, Mask=mask, ValueMask=valmask
        )
...
def generateCombinations(alphabet, combination_length):
    ...
    try:
        ...
        raise RuntimeError(
            "Bad parameter: alphabet must be of sequence "
    ...
    for curr_comb_num in xrange(num_combs):
        ...
        real_comb = []
        for position in curr_comb:
            real_comb.append(alphabet[position])
...
def makeNonnegInt(n):
    ...
    try:
        ...
        raise NonnegIntError(str(n) + " must be castable to a nonnegative int")
    # end try/except
...
def get_items_except(seq, indices, seq_constructor=None):
    ...
    result = [sequence[i] for i in range(len(seq)) if i not in index_lookup]
...
def NestedSplitter(
    ...
    def parser(line, index=0):
        ...
        if (
            ...
            and index > 0
            and len(result) == 1
            and isinstance(result[0], basestring)
        ):
            result = result[0]
...
def get_create_dir_error_codes():
    return {"NO_ERROR": 0, "DIR_EXISTS": 1, "FILE_EXISTS": 2, "OTHER_OS_ERROR": 3}
...
def handle_error_codes(dir_name, supress_errors=False, error_code=None):
    ...
    error_strings = {
        error_code_lookup["DIR_EXISTS"]: "Directory already exists: %s" % dir_name,
        error_code_lookup["FILE_EXISTS"]: "File with same name exists: %s" % dir_name,
        error_code_lookup["OTHER_OS_ERROR"]: "Could not create output directory: %s. "
        % dir_name
