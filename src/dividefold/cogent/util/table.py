...
import numpy
from dividefold.cogent.format import table as table_format, bedgraph

from dividefold.cogent.util.dict_array import DictArray
...
class Table(DictArray):
    ...
    def joined(
        ...
        **kwargs
        ...
        new_header = self.Header + [
            other_table.Title + "_" + other_table.Header[c] for c in output_mask_other
        ]
        if not joined_table:
    ...
    def transposed(self, new_column_name, select_as_header=None, **kwargs):
        ...
        data_indices = range(0, header_index) + range(header_index + 1, len(transposed))
