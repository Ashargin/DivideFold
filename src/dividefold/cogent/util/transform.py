...
from dividefold.cogent.maths.stats.util import Freqs
from dividefold.cogent.util.misc import identity, select
...
def make_trans(frm="", to="", default=None):
    ...
    if default is None:
        ...
        if len(default) != 1:
            raise ValueError("make_trans default must be single char: got %s" % default)
